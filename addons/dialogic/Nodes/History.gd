...
func add_history_row_event(eventData):
	...
	if eventData.has('character') and eventData.character != '':
		...
		if characterName != '':
			...
			if (lineBreakAfterName):
				lineBreak = '\n'
			characterPrefix = str("[color=",characterColor,"]", tr(parsed_name), "[/color]", charDelimiter, " ", lineBreak)
	...
	if eventData.event_id == 'dialogic_001':
		newHistoryRow.add_history(str(characterPrefix, tr(eventData.text.replace('[br]', '\n'))), audioData)
	# Character Arrivals
